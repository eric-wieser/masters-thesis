...
\subsection{Roll limits}
	...
	\Cref{fig:roll:progress} shows the effect of running the algorithm with three different roll limits, representing real (\SI{17}{\degree}) and idealized (\SI{90}{\degree}) scenarios, along with the scenario assuming an optimistic mechanical redesign (\SI{45}{\degree}).
	The difference in performance between \SI{17}{\degree} and \SI{45}{\degree} is minimal, showing that a mechanical redesign would not be a good uses of time.
	...
	\begin{figure}
		...
		\begin{subfigure}[t]{\linewidth}
			...
			\end{minipage}
			\caption{With cost penalty for roll}
		\end{subfigure}
	...
	One of the problems encountered due to roll limits is that cost function contains little information about the fact that the axle is about to hit the ground. A cost function should penalize undesirable behaviour, so a new term was added to $Q$ from \cref{eq:quad-cost}, of $\unitv{\theta}\frac{1}{\SI{17}{\degree}}^2\unitv{\theta}^T$. The results are shown in \cref{fig:roll:progress-cost}.

	\todo{Remark on these plots, once they exist}
\end{document}
